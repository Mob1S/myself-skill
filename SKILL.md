...
收到聊天记录后，在分析之前先执行时间预处理。

**2.1 时间提取**
- 从聊天记录中识别时间戳格式（如 `2024-03-15`、`2024/3/15`、`3月15日`、`昨天`/`上周` 等相对时间）
- 每条消息关联一个时间点
...
| 跨度 | 粒度 |
|---|---|
| < 3 个月 | 不分段，跳过时间预处理，直接进入 Step 3 走原始分析流程 |
| 3 个月 ~ 1 年 | 按月分段 |
| 1 ~ 3 年 | 按季度分段 |
| 3 年以上 | 按半年/年分段 |
...
时间跨度：{earliest} ~ {latest}（{duration}）
分段方式：{granularity}

| 时段 | 消息数 | 状态 |
|---|---|---|
| {period_1} | {count_1} | 正常 / 数据稀薄 |
| {period_2} | {count_2} | 正常 |
| ... | ... | ... |

断层：{gap_description}（如有）
```
...
时间预处理完成后（或跨度 < 3 个月直接进入），对每个时段独立执行六维度分析。**分析要具体**——每个结论都要有原文支撑。
...
```
📊 你的变化轨迹（{earliest} ~ {latest}）

▸ 高频词
  {period_1}: "{word_1}"、"{word_2}"       ← {style_label_1}
  {period_2}: "{word_3}"、"{word_4}"       ← {style_label_2}

▸ 句式
  {period_1}: 平均句长 {len_1} 字，{punctuation_style_1}
  {period_2}: 平均句长 {len_2} 字，{punctuation_style_2}

▸ 情绪表达
  {period_1}: {emotion_style_1}
  {period_2}: {emotion_style_2}

（仅展示有显著变化的维度）
```

**交互**：展示后询问"这些变化符合你的感受吗？"
...
> 跨度 < 3 个月时，跳过本步，直接进入 Step 5。

### Step 5：加权合并生成模型

**5.1 读取** `references/output-templates.md` 获取所有模板。

**5.2 加权合并**
- 默认衰减策略：越近的时段权重越高
- 衰减曲线根据时段数量自适应（不硬编码公式），原则是最新时段占主导但不完全丢弃历史
- 用户排除的时段权重归零
- 跨度 < 3 个月时，使用整体分析结果，无加权

**5.3 生成 `linguistic-fingerprint.md`**
- 保存到 `.claude/persona/linguistic-fingerprint.md`
- 跨度 ≥ 3 个月时：包含各时段分析结果 + 跨时段对比
- 跨度 < 3 个月时：包含整体 6 维度分析结果（与原逻辑一致）
...
- **必须包含时间轴段落**：记录各时段的权重分配（跨度 ≥ 3 个月时）
- **头部锚点字段**：`anchor_period` 设为最新时段（跨度 < 3 个月时设为"整体"）
...
├── model.md                      # 人格模型 V1（含 Layer 0 硬规则 + 运行规则 + Correction 记录 + 时间轴）
├── meta.json                     # 结构化元数据
├── rules.md                      # 调用规则
├── CHANGELOG.md                  # 版本日志
...
3. 对每个文件，逐个询问：
   - **时间信息**：该文件对应的时间段（如"2024 年上半年"、"最近三个月"），用于纳入时段权重
   - **权重设置**：
...
5. 按权重合并到现有模型：
   ...
   - **时段权重**：如果 model.md 含时间轴数据，新文件按其时间段纳入对应时段，更新时间轴
6. 更新 `model.md`，版本号 +0.1
7. 追加 `CHANGELOG.md`：（文件列表、权重设置、时间信息、主要变更摘要）
8. 输出合并摘要
...
3. 对指定内容执行**四层深度分析**：
   ...
   - **时段联动**：如果 model.md 含时间轴数据（跨度 ≥ 3 个月），深度分析按时段进行，补充到对应时段的分析结果中
4. 按维度**分组展示发现**，每个发现包含：原文证据（3+条）+ 新发现总结 + 与现有模型的差异（已有覆盖/新增/冲突）
5. 每组展示后询问用户确认（全部采纳 / 指定忽略 / 修改），逐组推进
6. 用户确认后更新文件：
   - `model.md`：增量合并，版本号 +0.1，更新时间轴中对应时段的数据
   - `linguistic-fingerprint.md`：底部追加 `## 深度分析补充 V{version}` 区块，保留原始分析，按时段归类
...
4. 读取 `model.md` 和 `person-profiles.md`，执行隐私过滤（真名脱敏、地点模糊化、转账删除、敏感事件抽象化、第三方脱敏）
   - **锚点感知**：默认使用 model.md 当前锚点时段的数据生成 soul.md。如果 model.md 含时间轴，告知用户当前锚点时段，用户可选择切换锚点后再导出
...
7. 超长裁剪优先级：熟人圈（泛泛之交 / 还算熟络）整段删 → 好友圈（要好的）精简 → 说话风格压缩 → 核心圈/底线不动
